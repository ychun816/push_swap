/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   cmd_rotate.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yilin <yilin@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/27 12:06:57 by yilin             #+#    #+#             */
/*   Updated: 2024/07/15 14:24:34 by yilin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"
/*
***rotate: The first element becomes the last one***

//Initial List
stack -> [1] <-> [2] <-> [3] <-> [4] -> NULL

//After Finding the Last Node
last_node -> [4]
stack -> [1] <-> [2] <-> [3] <-> [4] -> NULL

//Linking the Last Node to the First Node
{last_node} -> [4] -> [1] <-> [2] <-> [3] <-> [4] -> [1] (circular reference)
stack -> [1] <-> [2] <-> [3] <-> [4] -> [1] (circular reference)

//Updating the Head of the Stack
stack -> [2] <-> [3] <-> [4] -> [1] (circular reference)

//Setting the prev Pointer of the New Head to NULL
stack -> [2] <-> [3] <-> [4] -> [1] (circular reference)
       ^
       |
      NULL

//After Setting the next Pointer of the Old Head to NULL
stack -> [2] <-> [3] <-> [4] -> [1] -> NULL
       ^
       |
      NULL
*/
static void	rotate(t_stack **stack)
{
	t_stack	*og_head;
	t_stack	*last_node;
	
	if (!(*stack) || !(*stack)->next)//Check for Empty or Single-Node List
		return ;
	og_head = *stack;// Store the first node
	*stack = (*stack)->next;//Update head to the next node
	(*stack)->prev = NULL;//Set prev of new head to NULL
	og_head->next = NULL;//Extract/Isolate the old head node
	last_node = get_last_node(*stack);
	last_node->next = og_head;//Attach the old head node to the back
	og_head->prev = last_node;	
	// last_node->next = *stack; //Link the Last Node to the First Node
	// *stack = (*stack)->next;//Update the Head of the Stack
	// (*stack)->prev = NULL;//Set the prev Pointer of the New Head to NULL
	// (last_node->next)->prev = last_node;//Fix the prev Pointer of the Old Head
	// (last_node->next)->next = NULL;//Set the next Pointer of the Old Head to NULL
}

/*ra*/
void	ra(t_stack **a, int repeat)
{
	if (!(*a) || !repeat)
		return;
	rotate(a);
	ft_printf("ra\n");//write(STDOUT_FILENO, "ra\n", 3);
	ra(a, repeat - 1);// Recursively call ft_ra with rep decremented by 1
}
// void	ra(t_stack **a)
// {
// 	rotate(a);
// 	ft_printf("ra\n");//write(STDOUT_FILENO, "ra\n", 3);
// }

/*rb*/
void	ra(t_stack **b, int repeat)
{
	if (!(*b) || !repeat)
		return;
	rotate(b);
	ft_printf("rb\n");//write(STDOUT_FILENO, "rb\n", 3);
	rb(b, repeat - 1);// Recursively call ft_ra with rep decremented by 1
}
// void	rb(t_stack **b)
// {
// 	rotate(b);
// 	ft_printf("rb\n");//write(STDOUT_FILENO, "rb\n", 3);
// }

/*rr*/
void	rr(t_stack **a, t_stack **b, int repeat)
{
	if (!(*a) || !(*b) || !repeat)
		return ;
	rotate(a);
	rotate(b);
	ft_printf("rr\n");//write(STDOUT_FILENO, "rr\n", 3);
	rr(a, b, repeat - 1);
}
// void	rr(t_stack **a, t_stack **b)
// {
// 	rotate(a);
// 	rotate(b);
// 	ft_printf("rr\n");//write(STDOUT_FILENO, "rr\n", 3);
// }

/*
***rotate both: ***
simultaneously rotate two stacks (a and b) until one of the stacks reaches a specific node.

The loop will keep rotating both stacks until:
The head of stack b becomes cheapst_node->target_node.
The head of stack a becomes cheapst_node

Before rotate
Stack a: [1] <-> [2] <-> [3]
Stack b: [4] <-> [5] <-> [6]

After a few rotations,
Stack a: [2] <-> [3] <-> [1] (if cheapst_node was initially [1])
Stack b: [5] <-> [6] <-> [4] (if cheapst_node->target_node was initially [4])
*/
void rotate_both(t_stack **a, t_stack **b, int repeat1, int repeat2)
{
	if (repeat1 > repeat2)
	{
		rr(a, b, repeat2);// Perform repeat2 simultaneous rotations on both stacks
		ra(a, repeat1 - repeat2);// Perform the remaining rotations on the 'src(a)' stack
	}
	else
	{
		rr(a, b, repeat1);// Perform repeat1 simultaneous rotations on both stacks
		rb(b, repeat2 - repeat1);// Perform the remaining rotations on the 'b' stack
	}
}
// void rotate_both(t_stack **a, t_stack **b, t_stack *cheapest_node)
// {
// 	while(*a != cheapest_node && *b != cheapest_node->target_node)
// 		rr(a, b);//write(STDOUT_FILENO, "rr\n", 3);
// 	ft_current_position(*a);
// 	ft_current_position(*b);
// }
