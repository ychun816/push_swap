...
/*   Updated: 2024/07/12 18:08:01 by yilin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	rrotate(t_stack **stack)
{
	...
	(last_node->prev)->next = NULL;	//Detaching the Last Node
	last_node->next = *stack;//Link last_node to front
	last_node->prev = NULL;//update *stack to point to the new node; otherwise it points to old node
	*stack = last_node;	//update prev pointer; otherwise it still points to NULL => break the link
