...
/*   Updated: 2024/07/09 18:47:44 by yilin            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void push_a_to_b(t_stack **a, t_stack **b)
{
	...
	if (cheapest_node_a->above_median && cheapest_node_a->target_node->above_median)			// If both the cheapest `a` node and its target `b` node are above the median
		rrotate_both(a, b, cheapest_node_a);													//`rrotate_both` => execute if neither nodes are at the top
	else if (!(cheapest_node_a->above_median) && !(cheapest_node_a->target_node->above_median)) // If both the cheapest `a` node and its target `b` node are below the median
	{
		prep_for_push(a, cheapest_node_a, 'a'); // Ensure "cheapest nodes (a)" is at the top, ready for pushing
		prep_for_push(b, cheapest_node_a, 'b'); // Ensure ""target node (b)" is at the top, ready for pushing
		pb(a, b);								// pb
	}
}
// push b to a:
